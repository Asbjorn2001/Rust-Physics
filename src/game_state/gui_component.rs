...
pub trait GUIComponent {
    ...
    fn update(&mut self, cursor_pos: Vector2f<f64>, e: &Event, game: &mut Game) -> GUIEvent;
}

pub struct GUISlider2D {
    position: Vector2f<f64>,
    size: f64,
    pub value: Vector2f<f64>,
    pressed: bool,
    on_update: fn(Vector2f<f64>, GUIEvent, &mut Game) -> GUIEvent,
}

impl GUISlider2D {
    pub fn new(position: Vector2f<f64>, size: f64, on_update: fn(Vector2f<f64>, GUIEvent, &mut Game) -> GUIEvent) -> Self {
        Self { 
            position, 
            size, 
...
impl GUIComponent for GUISlider2D {
    ...
    fn update(&mut self, cursor_pos: Vector2f<f64>, e: &Event, game: &mut Game) -> GUIEvent {
        ...
        if self.pressed {
            ...
            return (self.on_update)(self.value, GUIEvent::Change, game);
        }

        (self.on_update)(self.value, GUIEvent::None, game)
    }
}
...
pub struct GUISlider {
    ...
    color: [f32; 4],
    pressed: bool,
    on_update: fn(f64, GUIEvent, &mut Game) -> GUIEvent,
}

impl GUISlider {
    pub fn new(position: Vector2f<f64>, size: Vector2f<f64>, color: [f32; 4], on_update: fn(f64, GUIEvent, &mut Game) -> GUIEvent) -> Self {
        Self { 
            position, 
            size, 
            value: 0.5, 
...
pub struct GUIButton {
    ...
    pub display: Display,
    pub is_hovered: bool,
    pub on_update: fn(&mut Self, GUIEvent, &mut Game) -> GUIEvent,
}
...
impl GUIButton {
    pub fn new(
        ...
        size: Vector2f<f64>, 
        display: Display, 
        on_update: fn(&mut Self, GUIEvent, &mut Game) -> GUIEvent) -> Self {
        Self { 
            position,
            size,
            display, 
...
impl GUIComponent for GUIButton {
    fn draw(&self, glyphs: &mut GlyphCache<'static, (), Texture>, c: Context, gl: &mut GlGraphics) {
    ...
    fn update(&mut self, cursor_pos: Vector2f<f64>, e: &Event, game: &mut Game) -> GUIEvent {
        ...
        if self.contains_cursor(cursor_pos) {
            if !self.is_hovered { 
                event = GUIEvent::Hover 
            }
            self.is_hovered = true;
            if let Some(Button::Mouse(MouseButton::Left)) = e.press_args() {
                event = GUIEvent::Click;
            }    
        } else {
            if self.is_hovered {
                event = GUIEvent::UnHover;
            }
    ...
    pub fn draw(&self, position: Vector2f<f64>, size: Vector2f<f64>, glyphs: &mut GlyphCache<'static, (), Texture>, c: Context, gl: &mut GlGraphics) {
        ...
        match &self.content {
            ...
            DisplayContent::Body(body, tex) => {
                ...
                body.draw(c.transform.trans_pos(offset), &tex, c, gl);
            }
            DisplayContent::Image(tex) => {
                let img = Image::new().rect(rect);
                img.draw(tex.as_ref(), &c.draw_state, c.transform.trans_pos(position), gl);
            }
        }        
    }
}