...
impl From<&Game> for PauseState {
    fn from(value: &Game) -> Self {  
        ...
        let resume_button = UIButton::new(
            button_position, 
            button_size, 
            display,
            |btn, event, game| { 
                match event {
                    UIEvent::Click => return UIEvent::StateChange(Box::new(playing_state::PlayingState::from(&*game))),
        ...
        let reset_button = UIButton::new(
            ...
            |btn, event, game| { 
                match event {
                    UIEvent::Click => {
                        *game = Game::default();
                        return UIEvent::StateChange(Box::new(PlayingState::from(&*game)));
                    },
                    UIEvent::Hover => btn.display.rect.border = Rectangle::new_round_border(color::BLACK, 15.0, 2.0).border,
                    UIEvent::UnHover => btn.display.rect.border = Rectangle::new_round_border(color::BLACK, 15.0, 1.0).border,
                    _ => {}
        ...
        let exit_button = UIButton::new(
            ...
            |btn, event, game| {
                match event {
                    UIEvent::Click => return UIEvent::StateChange(Box::new(main_state::MainState::from(&*game))),
                    UIEvent::Hover => btn.display.rect.border = Rectangle::new_round_border(color::BLACK, 15.0, 2.0).border,
                    UIEvent::UnHover => btn.display.rect.border = Rectangle::new_round_border(color::BLACK, 15.0, 1.0).border,
                    _ => {}
                }
